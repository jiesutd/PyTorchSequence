...
class LSTMTagger(nn.Module):
    def __init__(self, data,  gpu = False):
        ...
        if self.bilstm_flag:
            self.hidden2tag = nn.Linear(2*data.HP_hidden_dim, data.label_alphabet.size())
        else:
            self.hidden2tag = nn.Linear(hidden_dim, data.label_alphabet.size())
    ...
    def _get_neg_log_likilihood_loss(self, inputs, tags, gpu):
        ...
        tag_scores, tag_seq = self.forward(inputs,gpu)
    ...
    def forward(self, inputs, gpu=False):
        ...
        return tag_scores, tag_scores.data.numpy().argmax(axis=1)